...
const titleCase = (family: string) =>
    family
        ...
        .filter(Boolean)
        .map((part) =>
            KNOWN_ACRONYMS.has(part.toLowerCase()) ? part.toUpperCase() : part.charAt(0).toUpperCase() + part.slice(1),
        )
        .join(" ");

export const displayName = (id: string) => {
    const tag = tagOf(id);
    return tag ? `${titleCase(familyOf(id))} ${tag.toUpperCase()}` : titleCase(familyOf(id));
};
