...
4. Rebuild: `fetchModelsDevSeed` (models.dev: description, temperature), `fetchAllSpecs` (`POST /api/show` per id + output-limit probe via `/api/chat`), `buildCatalogDoc` merges previous costs/peak costs/model metadata and stamps the confirmed reasoning tiers, `publishCatalog`.
