...
- **models-dev.ts.** Wraps `GET https://models.dev/api.json` (`MODELS_DEV_URL`), provider entry `ollama-cloud`. Exports: `fetchModelsDevSeed() => Map<string, SeedEntry> | undefined` (best-effort; outage/shape drift logs a warning and returns `undefined` rather than blocking a rebuild); `seedEntry(seed, id) => SeedEntry | undefined` (exact id, then `familyOf(id)` fallback); `seedEntryOf(entry)` (normalizes `description`/`temperature`, omitting absent fields).
- **reasoning-overrides.ts.** Exports `REASONING_OPTIONS`, the four effort tiers confirmed to work on every cataloged model; `assemble.ts` stamps them onto each model.
...
4. **models.dev.** `fetchModelsDevSeed` reads `ollama-cloud` models' description and temperature build-time only; per field, the seed wins over the previous artifact and absence omits the field.

## Integration

- Consumed by: `src/catalog` (`assemble.ts` uses `seedEntry`; `extract-rates.ts` uses the `PricingTable` type) and the entrypoints `src/update-catalog.ts` / `src/update-pricing.ts`; `src/extract` only touches `FetchImpl` from `src/lib/http.ts`, not this directory.
