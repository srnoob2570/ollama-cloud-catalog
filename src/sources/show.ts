...
export async function probeMaxOutput(id: string, impl: FetchImpl = fetch): Promise<number> {
    ...
    if (ok) throw new Error(`output-limit probe for ${id} unexpectedly succeeded`);
    if (status === 401) throw new AuthError(`output-limit probe for ${id} needs OLLAMA_API_KEY (HTTP 401)`);
    const match = MAX_OUTPUT_RE.exec(error);
    if (!match) throw new Error(`output-limit probe for ${id}: unrecognized error: ${error.slice(0, 200)}`);
...
const parameterCountOf = (info: ShowResponse["model_info"]): number | undefined => {
    ...
    for (const [key, value] of Object.entries(info))
        if (key === "general.parameter_count" && typeof value === "number" && value > 0) return value;
    return undefined;
};

const capability = (caps: string[] | undefined, name: string) => caps?.includes(name) ?? false;
...
export async function fetchAllSpecs(
    ...
    return mapWithConcurrency(ids, concurrency, async (id) => {
        try {
            ...
            if (!prior)
                throw new Error(`spec fetch failed for ${id} and no previous spec exists to fall back on`, {
                    cause: err,
                });
