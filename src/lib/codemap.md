...
- Exported functions:
  - `fetchResponse(url: string, init?: RequestInit, impl?: FetchImpl): Promise<{status: number; text: string}>` — non-2xx is data, not failure (the output-limit probe reads a 400 body); 30s `AbortSignal.timeout`; injects `user-agent: ollama-cloud-catalog`.
  - `fetchText(url, init?, impl?): Promise<string>` — throws on non-2xx.
  - `fetchJson<T>(url, init?, impl?): Promise<T>` — `fetchText` + `JSON.parse`, throws with cause on invalid JSON.
  - `mapWithConcurrency<T, R>(items: T[], limit: number, fn: (item: T, i: number) => Promise<R>): Promise<R[]>` — shared-cursor worker pool, order-preserving; bounds the `/api/show` sweep.
  - `CHAT_URL: string` (`https://ollama.com/api/chat`), `ollamaAuthHeader(): string | undefined` — `Bearer $OLLAMA_API_KEY` or `undefined`; auth optionality is a caller decision (spec sweep tolerates absence, extraction requires it).
  - `modelsHash(models: {id: string; created: number}[]): string` — SHA-256 over sorted `id:created` pairs joined by `|`.
  - `stableStringify(value: unknown): string` — recursive key-sorted JSON, 2-space indent, trailing newline (stable diffs).
  - `writeAtomic(path: string, contents: string): Promise<void>` — write `<path>.tmp` via `Bun.write`, then `rename` over the target.
  - `familyOf(id: string): string`, `tagOf(id: string): string`, `displayName(id: string): string` — split on `:`; titleCase with `KNOWN_ACRONYMS` set (`gpt`, `oss`, `glm`, `llm`) uppercased, tag uppercased.
