...
- **Pure decision functions** (testable, no I/O): `decideRebuild(previous, liveHash, now, force): RebuildDecision` in `gate.ts` (reason precedence force > stale > hash-changed, `STALE_AFTER_MS` = 7 days, strict staleness); `refreshDecision(previousPricing, catalog, costById, peakCostById, peak?): RefreshDecision` in `assemble.ts` — discriminated unions `{ skip } | { repair } | { publish }`, rate signatures via `stableStringify` excluding volatile fields.
- **Builders/merge-back** (`assemble.ts`): `buildCatalogDoc(sources: CatalogSources)`, `buildPricingDoc(costById, peak?)`, `applyCosts(catalog, costById, peakCostById)` — refresh cost fields in place (cost map only refreshes; peak map is authoritative); reasoning seed from models.dev wins over prior artifact, absent everywhere → field omitted.
...
- **Resolution** (`resolve-rates.ts`): `resolveRateId(rateName, catalogIds)` — exact id, then unique `familyOf` match, ambiguous family throws; `resolveRates(rates, models, requireAll = true): RateResolution` collects problems instead of dropping silently.
