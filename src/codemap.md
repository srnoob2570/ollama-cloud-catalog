...
- `update-catalog.ts` — hash-gated spec pipeline; rebuilds the models.dev-shaped catalog from the live model list, preserving cost fields from the previous artifact (merge-back).
...
## Flow
`update-catalog.ts` — `bun src/update-catalog.ts <check|update> [--force]` (bad mode → usage + exit 2):
...
3. `update`: `decideRebuild` — `skip` prints "nothing to do" and exits 0; otherwise logs the reason (`force` | `stale` | `no-previous` | `changed`) and proceeds.
