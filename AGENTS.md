...
- **Publishing runs on Dokploy, not GitHub Actions.** `Dockerfile` + `docker/run-pipeline.sh` deploy as a Dokploy Application whose `CMD` is `sleep infinity`; Schedule Jobs exec `run-pipeline update` every 10 minutes (it chains `pricing` in the same run when the catalog changed), `run-pipeline force` Mondays 05:00 UTC, and `run-pipeline pricing` Mondays 06:00 UTC. The script clones `main` fresh and owns the commits (`chore: refresh ...`). Do not edit `catalog.json`/`pricing.json` manually. The old update workflows still exist during cutover and are deleted after one verified cycle.
