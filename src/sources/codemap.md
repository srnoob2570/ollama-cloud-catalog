...
- **models-dev.ts.** Wraps `GET https://models.dev/api.json` (`MODELS_DEV_URL`), provider entry `ollama-cloud`. Exports: `fetchModelsDevSeed() => Map<string, SeedEntry> | undefined` (best-effort; outage/shape drift logs a warning and returns `undefined` rather than blocking a rebuild); `seedEntry(seed, id) => SeedEntry | undefined` (exact id, then `familyOf(id)` fallback); `seedEntryOf(entry)` (normalizes effort tiers plus `description`/`temperature`, omitting absent fields); `reasoningOptionsOf(entry)` (filters `reasoning_options` with `type === "effort"`).
...
4. **models.dev.** `fetchModelsDevSeed` reads `ollama-cloud` models' effort tiers, description and temperature build-time only; per field, the seed wins over the previous artifact and absence omits the field.
