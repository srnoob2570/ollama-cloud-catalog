...
2. Rebuild path: spec sweep via `src/sources/`, then `buildCatalogDoc` merges specs + `previousCosts`/`previousPeakCosts`/`previousReasoningOptions`/`previousModelMeta` (merge-back) + models.dev seed (effort tiers, description, temperature), `CatalogDocSchema.parse`, `publishCatalog` → `writeAtomic` (temp file + rename).
